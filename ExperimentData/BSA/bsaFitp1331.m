...
s1=Spin( 0.87,      37.4998,   0.0741,  [0 0 2.9468e+07] );  %water
s2=Spin( 1.10,   2.3117e+03,  -3.8539,  [0 0 2.1308e+05] );
s3=Spin( 1.10,   1.8200e+03,   2.4773,  [0 0 5.6116e+05] );
...
sim.add_kex(2,1,   14.7443, 2, 20);
sim.add_kex(2,3,   3.4575);
sim.add_kex(3,1, 139.2932, 70, 200);
...
s1.opt_vals('R2') =  1; s1.set_lb_ub('R2',5, 50); 
s2.opt_vals('R2') =  1; s2.set_lb_ub('R2',  1e3, 5e3); 
s3.opt_vals('R2') =  1; s3.set_lb_ub('R2',  1e3, 4e3);
s4.opt_vals('R2') =  1; s4.set_lb_ub('R2', 1.e3, 8e3);


s1.opt_vals('c') =  1; s1.set_lb_ub('c',  1e4,  1e8 );
s2.opt_vals('c') =  1; s2.set_lb_ub('c',  100,  1e6 );
s3.opt_vals('c') =  1; s3.set_lb_ub('c',  100,  1e6 );
s4.opt_vals('c') =  1; s4.set_lb_ub('c', 1.0e3, 1.0e9);


s1.opt_vals('x') =  1; s1.set_lb_ub('x',-0.1,0.1);
s2.opt_vals('x') =  0; s2.set_lb_ub('x',-5.0, -2.2);
s3.opt_vals('x') =  0; s3.set_lb_ub('x', 1.2, 3.2);
